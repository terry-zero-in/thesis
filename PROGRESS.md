...
| THS-6 | Ticker detail page (chart + fundamentals) | ⬜ Todo (PR-C step 2 pending) | [#9](https://github.com/terry-zero-in/thesis/pull/9) MERGED 2026-05-02 + [#10](https://github.com/terry-zero-in/thesis/pull/10) MERGED 2026-05-02 (vitest infra) + [#11](https://github.com/terry-zero-in/thesis/pull/11) MERGED 2026-05-02 (ConvictionBadge → ui/) | **S240 (2026-05-02): scaffold creative-build proposal cycle OPENED.** Sections (1)-(6) drafted; **all 14 G + 5 GN + 4 Q (route-shell pre-impl) gaps locked** (durable rationale digest in HANDOFF carry-forward "S240 G/GN/Q rationale digest"). Two precursor PRs shipped: **PR #10** vitest infra (`4554e85`, retroactive Linear THS-17 Done) — devDeps vitest 4.1.5 + @testing-library/react 16.3.2 + jest-dom 6.9.1 + jsdom 29.1.1, vitest.config.ts (jsdom env + `@/*` alias), tests/smoke.test.ts. **PR #11** ConvictionBadge → `components/ui/` shared primitive (`75613fb`) — `git mv`, prop `value`→`score`, format `9/10` unchanged, single it.each test (5 cases), prettier in-scope hunks on watchlist-table.tsx. **PR-C step 2 (route shell) HELD on Q4-revisited:** Next 16 v16.2.0 added `unstable_retry` prop to error.tsx (recommended over legacy `reset`); 3 options surfaced awaiting Terry's A/B/C lock. Pre-write verifications done: validateTicker server-component compatible, MASSIVE_API_KEY LIVE on disk (length=32, AAPL probe HTTP 200, gotcha #14 stale), Next 16 docs locked except Q4-revisited. |
