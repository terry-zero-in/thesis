...
1. **Sanity check at session start.** `cd /Users/terryturner/Projects/thesis && git fetch --all && git status && gh pr list --state open && git log origin/main --oneline -5`. Expect `main` HEAD `909e49b` (PR #9 squash-merged 2026-05-02 S239) plus the HANDOFF metadata-refresh commit on top. PR #9 MERGED — branch `ths-6-fmp-stable-rebuild` deleted on origin. Via Linear MCP confirm THS-6 = Todo, THS-15 = Backlog, THS-7 = Todo, THS-16 = Done. Confirm dev server: `lsof -ti tcp:3000` — start `pnpm dev` if absent. Confirm Supabase: `supabase status` — `supabase start` if absent.

2. **PR #9 merge gate.** ✅ DONE 2026-05-02 (S239). Squash-merged at `909e49b` (`gh pr merge 9 --squash --delete-branch`); branch deleted on origin + local pruned. Codex NOT engaged per durable rule. Linear THS-6 stays Todo (page work is the actual ticket scope, not lib infra).

3. **Run BRK-B form curl probe** (queued cycle item 4). ✅ DONE 2026-05-02 (S239). Both `/stable/key-metrics` and `/stable/ratios` returned identical HTTP 402 + body `Premium Query Parameter: 'Special Endpoint : This value set for 'symbol' is not available under your current subscription...` — exact match on `FMPSymbolNotCoveredError` body regex. **Resolution: A bucket — BRK.B genuinely uncovered on /stable/ free tier, no form-transform layer needed.** Q-STORAGE U (storage = hyphen form) stands unchanged. Same gap class as NET / QQQ / MDB.

4. **Fix HANDOFF.md staleness** (queued cycle item 5). ✅ DONE 2026-05-02 (S239) — THIS COMMIT. Refreshed HANDOFF + PROGRESS to current `origin/main` = `909e49b` (post-PR #9 merge). All body refs to `7f35d25` / `d88f94e` / `ths-6-fmp-stable-rebuild` updated. Per `feedback_handoff_metadata_staleness.md` memory rule. Direct to main, no PR.

5. **Open THS-6 scaffold creative-build proposal cycle** (queued cycle item 6). ☐ Awaits Terry's go — preconditions 1-5 ALL CLEARED post-merge sequence S239. Per HANDOFF "THS-6 scaffold proposal cycle — preconditions before opening" subsection. Sections 1-6 per creative-build hard-gate. Latest research block (item 11) + extracted conviction badge (item 12) are line items inside the proposal, NOT pre-approved scope. Fundamentals tab needs graceful "not available on current data plan" UI surface for `FMPSymbolNotCoveredError` cases (NET / BRK-B / QQQ / MDB).
...
> **Refer to `HANDOFF.md` and `PROGRESS.md` in `/Users/terryturner/Projects/thesis/` for full session context, file paths, and the NEXT 3-5 TASKS block. Main HEAD is `909e49b` (PR #9 squash-merged 2026-05-02 S239 — `feat(fmp): rebuild getKeyMetrics against /stable/ surface`) plus the HANDOFF metadata-refresh commit on top. PR #9 MERGED. NET coverage decision = A LOCKED — accept gap, modify Phase 1 acceptance criterion. BRK.B form probe RESOLVED 2026-05-02 (S239): A bucket — both `/stable/key-metrics` and `/stable/ratios` return HTTP 402 + symbol-not-covered for `?symbol=BRK.B`; no transform layer needed; same gap class as NET / QQQ / MDB. THS-6 spec lock recorded in HANDOFF carry-forward (items 5-13 + design principles a-d). 13 carry-forward items + 4 design principles + 6-step scaffold-cycle preconditions checklist (items 1-5 ALL CLEARED, item 6 awaits Terry's go). THS-6 page work resumes when scaffold proposal cycle opens with creative-build hard-gate. Run SESSION-STARTUP SANITY CHECKS, then ASK Terry how to proceed — do not auto-assign per `feedback_no_task_assignment.md`.**
