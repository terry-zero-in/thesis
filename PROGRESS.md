...
| THS-6 | Ticker detail page (chart + fundamentals) | ⬜ Todo | [#9](https://github.com/terry-zero-in/thesis/pull/9) MERGED 2026-05-02 | Lib infrastructure prep shipped via [#8](https://github.com/terry-zero-in/thesis/pull/8) at `c72c6f2` 2026-05-01. **Q-FMP-KEY cleared S237 2026-05-02 → /api/v3/key-metrics deprecated → rebuilt on PR #9 squash-merged at `909e49b` 2026-05-02 (S239)** — `getKeyMetrics` against `/stable/`, sibling error class hierarchy, 3 helper-probes PASS, 4/10 watchlist tickers (NET/BRK-B/QQQ/MDB) hit `FMPSymbolNotCoveredError`. NET coverage decision A LOCKED (accept gap; Phase 1 acceptance criterion modified). BRK.B form probe RESOLVED S239: A bucket (no transform layer needed; same gap class as NET / QQQ / MDB). 13 of 13 THS-6 spec items LOCKED in HANDOFF carry-forward (tab structure, Latest research block, design principles, etc.). Page work resumes when scaffold proposal cycle opens — preconditions ALL CLEARED, awaits Terry's go. |
