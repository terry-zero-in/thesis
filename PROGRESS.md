...
| THS-6 | Ticker detail page (chart + fundamentals) | ⬜ Todo (PR #12 DRAFT in flight, **6 commits accumulated** — steps 2/3/4 + 1 step-3 polish + 2 step-4 polish) | [#9](https://github.com/terry-zero-in/thesis/pull/9) MERGED + [#10](https://github.com/terry-zero-in/thesis/pull/10) MERGED + [#11](https://github.com/terry-zero-in/thesis/pull/11) MERGED + [#12](https://github.com/terry-zero-in/thesis/pull/12) DRAFT | **S243+S244 (2026-05-02 → 2026-05-03): PR-C step-3 polish + step 4 + 2 step-4 polish commits shipped to draft PR #12.** Step-3 polish (`6178fdb`): friendly exchange label mapper at `lib/market/exchange.ts` mapping ISO 10383 MIC codes (XNAS, XNYS, ARCX, BATS, IEXG) to institutional labels (NASDAQ, NYSE, NYSE Arca, Cboe BZX, IEX) with raw MIC fallback for unmapped; wired into TickerHeader; dropped `uppercase` Tailwind class on exchange span so mixed-case labels render per spec; vitest 6/6 (5 mapped + 1 fallback). Step 4 (`d8fd650`): shadcn `tabs` + `skeleton` primitives installed (`components/ui/{tabs,skeleton}.tsx`); `components/ticker/tabs.tsx` composition with 6 tabs `[Overview, Chart, Fundamentals, Research History, Triggers, Thesis]` (Q1 PATH B locked — Triggers from day one), `useState<TabId>` + `useTransition` for client-side swap (Q2 spec-locked from G10+G14, no URL persistence), default Overview (Q3 G10), persistent chrome above (TickerHeader+PriceBlock visible regardless of active tab), Chart panel = wide-rect skeleton with `animate-pulse` stripped per `feedback_empty_state_asymmetry.md`, Fundamentals panel = 5-cell KPI grid skeleton (G7 shape) pulse stripped, Research/Triggers/Thesis = prose with locked ticket citations (`No research yet. Available with THS-7.` / `No active triggers. Evaluator ships with THS-9.` / `Thesis tracking ships post-Phase 1.`), Overview content panel empty (lands in step 5+); active style = `text-text-1` + 1px override on `after:` pseudo-element with `!bottom-0 !h-px !bg-accent`; tab strip `px-8` matches TickerHeader; tablist `border-b border-border-subtle`. Step-4 polish 1 (`a3a81f7`): `flex-none` override added to TRIGGER_CLASSES so tabs group left at content-width (shadcn primitive's `flex-1` was distributing each tab to 1/6 of row — Linear pattern is content-width grouped left, not full-width-distributed). Step-4 polish 2 (`07348d5`): `!h-px → !h-0.5` (1px → 2px) per legibility floor on dark canvas — 1px reads as structural hairline, 2px reads as state. Verifications at every checkpoint: tsc silent-clean + vitest 12/12 + build clean + `/tickers/[symbol]` confirmed `ƒ` Dynamic in route table. **Two new Linear tickets filed via Linear MCP this session:** THS-21 (Backlog Medium — display-layer string normalization broad-scope; sibling of THS-19/20 polish bucket; NO per-field tickets), THS-22 (Backlog Low — tabs primitive cleanup to drop !important override layer; sibling of THS-18/19/20/21). Step 4 visual eyeball CONFIRMED for layout (post-`a3a81f7` flex-none fix), 2px underline thickness (post-`07348d5`) NOT yet visually confirmed. Verification surface = localhost:3000 (Vercel preview 500s by gotcha #23). |
...
### Session 2026-05-02 → 2026-05-03 (Claude Code #246_05.02.2026 — S243+S244 — PR-C step-3 polish + step 4 + 2 step-4 polish + THS-21 + THS-22 + 3 new memory rules)

**Focus:** Verify Terry's eyeball on PR-C step 3 → ship step-3 polish (friendly exchange label mapper) → run creative-build HARD-GATE for step 4 (tab strip with 6 tabs, Path B locked, persistent chrome layout) → ship step 4 with 2 polish commits to fix layout (flex-1) and visibility (2px underline). File 2 Linear tickets (THS-21 display-layer normalization broad-scope, THS-22 tabs primitive cleanup). Save 3 new durable cross-project memory rules (spec-citation-before-drift, empty-state asymmetry + shadcn skeleton dual-use, 2px floor for active-state indicators on dark canvas). Take one terminal feedback hit from Terry: don't over-polish during scaffold phase.

**Done:**
- **PR-C step-3 polish shipped to PR #12 (`6178fdb`).** `lib/market/exchange.ts` mapper: XNAS→NASDAQ, XNYS→NYSE, ARCX→NYSE Arca, BATS→Cboe BZX, IEXG→IEX. Raw MIC fallback for unmapped (visible regression signal). `lib/market/__tests__/exchange.test.ts` covers 6 cases (5 mapped + 1 fallback). Wired into TickerHeader; dropped `uppercase` Tailwind class on exchange span so mixed-case labels render per spec. Vitest 12/12 (existing 6 + new 6). Step 3 visual eyeball CONFIRMED post-polish: NVDA renders with NASDAQ instead of XNAS.
- **PR-C step 4 shipped to PR #12 (`d8fd650`).** shadcn `tabs` + `skeleton` primitives installed via `pnpm dlx shadcn@latest add tabs skeleton` (created `components/ui/{tabs,skeleton}.tsx`, no new deps). `components/ticker/tabs.tsx` composition: 6 tabs in declared order, `useState<TabId>` + `useTransition` (no URL persistence per G10+G14 joint lock), default Overview, persistent chrome above (TickerHeader+PriceBlock visible regardless of active tab — Linear/Bloomberg/Stripe/Mercury pattern). Empty-state implementation per asymmetry rule: Chart panel = wide-rect skeleton (animate-pulse stripped + JSX comment documenting intent), Fundamentals panel = 5-cell KPI grid skeleton (G7 shape, pulse stripped), Research/Triggers/Thesis = prose with locked ticket citations. Active style = `text-text-1` + 1px override on `after:` pseudo-element with `!bottom-0 !h-px !bg-accent`. Tab strip `px-8` matches TickerHeader; tablist `border-b border-border-subtle`. Wired in `app/(app)/tickers/[symbol]/page.tsx` below persistent chrome. Build clean, `/tickers/[symbol]` `ƒ` Dynamic.
- **PR-C step-4 polish 1 shipped to PR #12 (`a3a81f7`).** Caught from Terry's first step-4 screenshot: tabs were full-width-distributed (each at 1/6 of row) because shadcn TabsTrigger primitive ships with `flex-1` which I didn't override. Added `flex-none` to TRIGGER_CLASSES so tabs group left at content-width per Linear pattern. Comment in tabs.tsx documents the override intent.
- **PR-C step-4 polish 2 shipped to PR #12 (`07348d5`).** Per Terry's lock after second screenshot: bumped active-tab indicator from `!h-px` (1px) to `!h-0.5` (2px in Tailwind v4) per legibility floor on dark canvas. Color (`--accent`) and position (`bottom-0`) unchanged. Inactive styling untouched. Comment expanded to document the 2px choice.
- **THS-21 filed via Linear MCP** — Backlog Medium, Phase 1 — MVP project. Title: "Display-layer string normalization: Massive raw → institutional convention." Broad-scope ticket: company name caps/abbrev mappers (NVIDIA, IBM, AMD, ARM, AT&T, JPMorgan; Corp→Corporation, Inc→Inc.), ticker symbol casing edge cases (BRK.B vs BRK-B), CUSIP/ISIN formatting Phase 2+, share class formatting. **NO per-field tickets** — all display-layer surface lifts ride into THIS ticket. Initial trigger: "Nvidia Corp" → "NVIDIA Corporation". Sibling of THS-19/20 polish bucket pre-Phase-1-close.
- **THS-22 filed via Linear MCP** — Backlog Low, Phase 1 — MVP project. Title: "Tabs primitive: edit shadcn defaults to drop !important override layer." Edit `components/ui/tabs.tsx` defaults from `bottom-[-5px] h-0.5 bg-foreground` → `bottom-0 h-0.5 bg-accent` so the override clause in `components/ticker/tabs.tsx` becomes deletable. Sibling of THS-18/19/20/21 polish bucket. shadcn explicitly supports primitive edits.
- **Three new durable cross-project memory rules saved.**
  - `feedback_spec_citation_before_drift.md` — pull DESIGN_SPEC citation FIRST before raising "should we swap X" on locked primitives. Origin: I called Geist Sans scaffold drift; Terry corrected — `docs/design/DESIGN_SPEC.md:98-109` locks Geist + JetBrains Mono explicitly.
  ...
  - `feedback_active_state_indicator_2px_floor.md` — 2px floor for active-tab underlines, selected-row rails, active-nav bars on dark canvas. 1px reserved for structural borders (border-subtle dividers, hairlines) only. State indicators never get "barely there" treatment. Linear / Stripe Dashboard precedent.

**Decisions Terry locked (do not relitigate):**
- **Q1 PATH B** — 6 tabs final at `[Overview, Chart, Fundamentals, Research History, Triggers, Thesis]`. Triggers tab present from day one with empty-state copy through THS-9; NOT folded into Overview.
- **Q2 spec-locked** — `useState<TabId>` + `useTransition`. No URL persistence on tab switch. Refresh on a non-Overview tab returns to Overview. Per G10 + G14 joint lock.
- **Q3 spec-locked** — Default tab = Overview. Per G10.
- **Q4 split lock** — Skeleton (Path 1) for transient Chart + Fundamentals; prose+ticket for durable Research/Triggers/Thesis.
- **Layout** — Persistent ticker chrome above tab strip. Chrome stays, body swaps.
- **Active style** — 2px `--accent` bottom border + `text-text-1` (NOT 1px); inactive `text-text-3`, hover `text-text-2`. NO filled pills, NO transition/animation on indicator.
- **Display-layer normalization scope** — single broad-scope ticket (THS-21), NOT per-field tickets.
- **Tabs primitive cleanup deferred** — THS-22 ships standalone post-PR-C. Do NOT refactor primitive in this PR.
- **Geist Sans + JetBrains Mono** — locked per `docs/design/DESIGN_SPEC.md:98-109`. NOT scaffold drift. Do NOT propose Inter swap, ever.
- **Self-referential ticket citation BANNED** — never cite the in-flight ticket from inside its own preview surface.
- **Don't over-polish during scaffold** — terminal feedback. Ship functional first, fine-tune later.

**Closeout-side gap:** Reticle Supabase env vars (`SUPABASE_URL` / `SUPABASE_KEY`) empty in shell. `session_activity` row PATCH could not run from CLI — Terry to update via Hub or set env vars and re-run inline.

**Next-session blockers:**
1. Sanity check (per always-pattern). Verify SHA matches HANDOFF before trusting text.
2. Verify Terry's eyeball on the 2px underline (`07348d5`) — last screenshot showed 1px state.
3. Step 5 (chart wiring) HARD-GATE on Terry's "step 5 go." Q-CHART-LIB will surface at the gate (locked stack does not name a chart lib).
4. THS-20 (topbar 4-state pill backfill) still queued post-PR-C merge. Do NOT auto-pick — Terry locks the trigger.
5. Reticle Supabase env-var gap surface point at startup.
